...
fit_curve (curve_type curve)
{
  ...
  if (CURVE_LENGTH (curve) < 4)
    fitted_splines = fit_with_line (curve);
  else
    {
      ...
      find_tangent (curve, /* to_start_point: */ true,
                    /* cross_curve: */ false);
      find_tangent (curve, /* to_start_point: */ false,
                    /* cross_curve: */ false);

      set_initial_parameter_values (curve);

      fitted_splines = fit_with_least_squares (curve);
    }
...
/* A ``knee'' is a point which forms a ``right angle'' with its
   predecessor and successor.  Either its predecessor differs only in x,
   and its successor only in y, or its predecessor differs only in y,
   and its successor only in x.  (Hence the `ONLY_ONE_ZERO' macro
   below.)  For example:
   *
   X*  The point `X' is a knee, if we're moving in a clockwise
   direction.  The argument CLOCKWISE tells us which direction we're
   moving.  (We can't figure that information out from just the single
   segment with which we are given to work.)  We should never find two
   consecutive knees.

   Such a point can be on either the inside or the outside of the
   outline.  Points on the inside do nothing useful, they just slow
   things down and, more important, make the result fit a less accurate
   curve.  So we remove them.  But points on the outside help to define
   the shape of the curve, so we leave those.
...
/* This code isn't needed anymore, since we do the filtering in a
   somewhat more general way.  */
          ...
          /* Put the newly computed point into a separate curve, so the
             points we filter don't affect future computation on this
             iteration.  */
  ...
  while (true)
    {
      ...
      /* Don't exit, even if `error_ok', because we might be able to
         improve the fit even more with further reparameterization.  If
         the reparameterization made it worse, we will exit here, since
         `improvement' will be negative.  */
...
/* Our job here is to find alpha1 (and alpha2), where t1_hat (t2_hat) is
   the tangent to the curve at the starting (ending) point, such that: 

   CONTROL1 = alpha1*t1_hat + starting point
   CONTROL2 = alpha2*t2_hat + ending_point

   See pp.58--59 of the Phoenix thesis.  The B?(t) here corresponds to
   B_0^3(U_i) there.  The Bernshte\u in polynomials of degree n are defined by
   B_i^n(t) = { n \choose i } t^i (1-t)^{n-i}, i = 0..n  */

#define B0(t) ((1-(t))*(1-(t))*(1-(t)))
#define B1(t) (3.0*(t)*(1-(t))*(1-(t)))
#define B2(t) (3.0*(t)*(t)*(1-(t)))
#define B3(t) ((t)*(t)*(t))
...
fit_one_spline (curve_type curve)
{
  ...
  for (i = 0; i < CURVE_LENGTH (curve); i++)
    {
      vector_type *Ai = A[i];
      vector_type start_vector = make_vector (START_POINT (spline));
      vector_type end_vector = make_vector (END_POINT (spline));
  ...
  if (epsilon_equal (C0_C1_det, 0.0))
    FATAL ("determinant of C0*C1 is zero");
...
/* If the D component of the vector V is smaller than the best so far,
...

/* When we have finished fitting an entire character, we want to go
   through the spline list L to ensure that any endpoints that are
   ``close enough'' (i.e., within `align_threshold') to being the same
   really are the same.  */
...
/* Unfortunately, we cannot tell in isolation whether a given spline
   ...
   outline between two corners.)  After subdividing the curve, a line
   may very well fit a portion of the curve just as well as the
