/* pathsearch.h: environment-variable path searching for files, possibly
   in subdirectories.
...
/* Return a NULL-terminated array of all the directory names in the
   ...
   If CWD_FIRST_P is set, `./' will be the first element in
   the returned list.  */
extern string *initialize_path_list P3H(string env_name,
                                        string default_path,
                                        boolean cwd_first_p);
