...
/* Printer's points, as defined by TeX.  */
...
#define POINTS_TO_PIXELS(v, dpi) ((int) ((v) * (dpi) / POINTS_PER_INCH + .5))
...
/* Says whether V1 and V2 are within REAL_EPSILON of each other.
   ...
   independence, but it's so much more painful to work with.  */
...
extern one_byte parse_charcode (string s, boolean *valid);

/* Like `parse_charcode', but gives a fatal error if the string isn't a
   valid character code.  */
extern one_byte xparse_charcode (string s);
