...
/* Standalone (not too thorough) check for corruption.  */
void malloc_sanity_check(void* mem) 
{ 
...
static inline void malloc_unlink(mchunkptr p)
{
  mchunkptr b = p->bk;
  mchunkptr f = p->fd;
...
static mchunkptr malloc_from_sys(unsigned nb)
{
  ...
  *last_sbrk_end = SIZE_SZ | INUSE;
  ...
  /* make it safe to malloc_unlink in malloc */
  UPDATE_STATS(++n_avail);
  p->fd = p->bk = p;
  ...
  for (b = malloc_maxbin; b >= FIRSTBIN; --b)
  {
    ...
    if (b->dirty)
    {
      ...
      while (p != h)
      {
        ...
        int consolidated = 0;          /* only malloc_unlink/relink if consolidated */
        ...
        {
          if (!consolidated) { consolidated = 1; malloc_unlink(p); }
  ...
  while (p != hd)
  {
    ...
    else    /* try to consolidate; same code as malloc_find_space */
    {
      ...
      if (consolidated)
      {
        if (p->size >= nb)
        {
          /* make it safe to malloc_unlink again below */
          UPDATE_STATS(++n_avail);
          p->fd = p->bk = p;
          goto found;
        }
