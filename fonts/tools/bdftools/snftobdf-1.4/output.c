...
TempFont *tf;
{
    ...
    for (i = 0; i < tf->pFI->nProps; i++) {
        ...
        else if (!strcmp(tf->pFP[i].name, "RESOLUTION"))
          res = tf->pFP[i].value;
        else if (!strcmp(tf->pFP[i].name, "RESOLUTION_X"))
          resX = tf->pFP[i].value;
        else if (!strcmp(tf->pFP[i].name, "RESOLUTION_Y"))
          resY = tf->pFP[i].value;
    }
...
char *name;
{
    ...
    CharInfoPtr minBounds = &(pFI->minbounds);
    CharInfoPtr maxBounds = &(pFI->maxbounds);
    int props;
    int b1, b2, b3, b4;
    time_t clock;
