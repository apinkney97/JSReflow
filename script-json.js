...
function getLine(words) {
	...
	for (var i = 0; i < length; i++) {
	  offset = words[i][0];
	  word = words[i][1];
	  text += "<div class=\"inner\" style=\"left: " + offset * SCALE + "pt;\">" + DICTIONARY[word] + "</div>";
	}
	text += "</div>";
    ...
    */
    ...
    for (p = 0; p < PARAGRAPH_TREE.length; p++) {
        ...
        if (Array.isArray(PARAGRAPH_TREE[p])) {
            ...
            w = PARAGRAPH_TREE[p].w * SCALE;
            h = PARAGRAPH_TREE[p].h * SCALE;
            ...
            if (FLOAT_TYPE === FLOAT_TYPES.SIMPLEQUEUE) {
                ...
                placeFloat(w, h, nlines, span, "<div>" + PARAGRAPH_TREE[p].d + "</div>");
