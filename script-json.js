...
function getFloat(width, height, text) {
    "use strict";
    return getDiv("floatable", width * JSReflow.scale, height * JSReflow.scale, text);
}
...
function placeFloat(w, h, nlines, span, text) {
    ...
    JSReflow.columns[JSReflow.curr_col][JSReflow.curr_row] = getFloat(w, h, text);
    ...
    if (offset + JSReflow.curr_page > JSReflow.pages) {
        ...
        for (p = start; p < JSReflow.columns.length && p < end; p++) {
            ...
            for (l = 0; l < JSReflow.columns[p].length; l++) {
                ...
                outputDiv("container", ((JSReflow.col_sep - JSReflow.galley_width * JSReflow.scale) / 2 + (p - start) * JSReflow.col_sep), (JSReflow.top_margin + l * JSReflow.vs) * JSReflow.scale, JSReflow.columns[p][l]);
            }
        }
    ...
    for (p = 0; p < JSReflow.paragraph_tree.length; p++) {
        ...
        if (Array.isArray(JSReflow.paragraph_tree[p])) {
            ...
            for (l = 0; l < JSReflow.paragraph_tree[p][min_badness_index].length; l++) {
                nextPos();
                JSReflow.columns[JSReflow.curr_col][JSReflow.curr_row] = getLine(JSReflow.paragraph_tree[p][min_badness_index][l]);
            }
            nextPos();
            ...
            if (JSReflow.float_type === JSReflow.float_types.SIMPLEQUEUE) {
                ...
                placeFloat(w, h, nlines, span, "<div>" + JSReflow.paragraph_tree[p].d + "</div>");
...
$(window).resize(function () {
    ...
    JSReflow.resizetimer = setTimeout(doLayout, 200);
    doLayout();
});
